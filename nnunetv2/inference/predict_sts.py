...
from nnunetv2.training.nnUNetTrainer import STSTrainer
                    ...
                    r.append(
                        export_pool.starmap_async(
                            ...
                            ((prediction, properties, self.configuration_manager, self.plans_manager,
                              self.dataset_json, ofile, save_probabilities, True),)
                        )
                    )
                else:
                        ...
                        export_pool.starmap_async(
                            convert_predicted_logits_to_segmentation_with_correct_shape, (
                                (prediction, self.plans_manager,
                                 ...
                                 properties,
                                 save_probabilities, default_num_processes, True),)
                        )
                    )
                if ofile is not None:
...
if __name__ == '__main__':
    ...
    # folder0的半监督权重
    predictor.initialize_from_trained_model_folder(
        join(nnUNet_results, 'Dataset1002_STS3D/STSTrainer__nnUNetPlans__3d_fullres'),
        use_folds=(0, ),
        checkpoint_name='checkpoint_best_self_train.pth',
    )
    predictor.predict_from_files(join(nnUNet_raw, 'Dataset1002_STS3D/test'),
                                 join(nnUNet_raw, 'Dataset1002_STS3D/test_pred'),
