...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        outputs, img_features = model(samples)
...
@torch.no_grad()
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, plot_dir):
    model.eval()
    criterion.eval()
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        orig_target_sizes = torch.stack([torch.tensor([sample.shape[1], sample.shape[2]], device=sample.device) for sample in samples], dim=0)
        ...
        data_loader.dataset.draw_pred_result(samples[0], results[0], gt_boxes[0], gt_labels[0],
