...
from xxdet.datasets.quadrant_dataset import QuadrantDataset
...
def get_args_parser():
    ...
    # * Backbone
    # 我们这里不需要
    parser.add_argument('--[TBD]backbone', default='resnet50', type=str,
    ...
    parser.add_argument('--output_dir', default='D:\\xsf\\Dataset\\Oral_Panorama_Det\\training_data\\quadrant\\save_weights',
...
def worker(rank, world_size, args):
    ...
    if world_size > 1:
        ...
        dist.init_process_group(
            backend='gloo', init_method="file://D:\\xsf\\Dataset\\Oral_Panorama_Det\\training_data\\quadrant\\ddp.txt",
    ...
    # 这里的class只需要设置前景即可
    model, criterion, postprocessors = build_model(args, num_classes=1, rank=0,
    ...
    param_dicts = [
        {"params": [p for n, p in model_without_ddp.named_parameters() if "[TBD]backbone" not in n and p.requires_grad]},
        {
            "params": [p for n, p in model_without_ddp.named_parameters() if "[TBD]backbone" in n and p.requires_grad],
            "lr": args.lr_backbone,
        },
    ]
    ...
    # TODO: 用我们自己的dataset
    train_dataset = QuadrantDataset("D:\\xsf\\Dataset\\Oral_Panorama_Det\\training_data\\quadrant\\xrays",
                                    "D:\\xsf\\Dataset\\Oral_Panorama_Det\\training_data\\quadrant\\train_quadrant.json")
    ...
    # 这个我手动遍历来做部分预测
    val_dataset = QuadrantDataset("D:\\xsf\\Dataset\\Oral_Panorama_Det\\training_data\\quadrant\\xrays",
                                  "D:\\xsf\\Dataset\\Oral_Panorama_Det\\training_data\\quadrant\\train_quadrant.json")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 10 == 0:
            test_stats, coco_evaluator = evaluate(
                ...
                plot_dir="D:\\xsf\\Dataset\\Oral_Panorama_Det\\training_data\\quadrant\\val"
            )
