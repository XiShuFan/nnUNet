...
def plot_loss_and_lr(train_loss, learning_rate, file_dir):
    try:
        ...
        fig.savefig(os.path.join(file_dir, './loss_and_lr.png'))
        plt.close()
...
def plot_map(mAP, file_dir):
    try:
        ...
        plt.xlim(0, len(mAP))
        plt.legend(loc='best')
        plt.savefig(os.path.join(file_dir, 'mAP.png'))
        plt.close()
